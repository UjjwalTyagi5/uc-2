...
Return at most K survivors (or fewer if the pool truly has fewer matches —
NEVER pad with weak ones). Every candidate the model evaluated must appear
in either `selected` or `rejected` — no candidate may silently disappear.
Mathematically: |selected| + |rejected| == |candidates_received|.

Because every `selected` row has ALREADY passed the strict ±critical_pct
band on every source critical attribute (Rule 1b), the STRONG / MODERATE /
WEAK verdict reflects the WHOLE picture — important attribute penalties,
spec-to-price-ratio band, candidate-has-extra-attrs penalties, age, brand:

  STRONG_MATCH    — All source critical attrs within band, zero important
                    attribute penalties, spec-to-price ratio inside
                    ±{ratio_pct}% of pool median, no rule-1d penalty.
                    confidence ≥ 0.80. Use as a primary anchor.
  MODERATE_MATCH  — All source critical attrs within band, but 1–2 soft
                    penalties (important attr mismatch, brand drift, or
                    rule-1d extra attr). spec-to-price ratio may be
                    DOWNRANK zone. confidence 0.55–0.79. Use as
                    secondary corroboration of the strong matches.
  WEAK_MATCH      — All source critical attrs within band, but multiple
                    soft penalties AND/OR spec-to-price in DOWNRANK zone
                    AND/OR age >24 months. confidence 0.30–0.54. Use as
                    last-resort sanity check, not as primary anchor.

Note: a candidate OUTSIDE the ±critical_pct band is NEVER a WEAK_MATCH.
It is a REJECT. WEAK is reserved for candidates that pass the band but
carry multiple soft penalties.

For EACH selected row, produce a rich `agent_reasoning` payload so a buyer
can audit the match without re-running the model:
  - matched_attrs: every critical AND important attribute you compared,
    with BOTH the source value and the candidate value, the unit, the
    delta % (rounded to 1 decimal, round-half-to-even), the importance you
    assessed, and a per-attribute PASS / PENALTY verdict. List ALL
    attributes you considered — don't elide the ones that passed cleanly.
  - spec_to_price_ratio: when meaningful (a critical numeric attr +
    unit_price_eur both exist). Include the ratio, the surviving-pool
    median you used, and PASS / DOWNRANK / REJECT verdict. If not
    meaningful, set verdict="N/A" and value/pool_median to null.
  - penalties: short bullet-style strings, one per soft penalty. Be
    specific — "shot_weight 1150g vs source 1200g (4.2% below)" beats
    "minor spec drift". Cite numbers + units.
  - confidence: 0.0–1.0. Map from verdict per the bands above.
  - verdict: STRONG_MATCH | MODERATE_MATCH | WEAK_MATCH.
  - reasoning: a BUYER-FACING justification of 5–8 sentences, written so a
    procurement manager could paste it directly into a justification email
    to a CPO or finance reviewer. Structure it as four named beats —
    keep the labels visible in the prose so it reads as audit-grade:

      PRODUCT FIT: state that the candidate is the SAME product family
        (cite drive_type / sub_type / fuel_type / device_class) and the
        SAME item_level chain. Quote both source and candidate level_3.
      SPEC ALIGNMENT: quote every critical attribute by name with src,
        cand, unit, and delta %. Then quote the important attributes the
        same way. Numbers, not adjectives.
      PRICE COMPARABILITY: if a critical numeric attr exists, quote the
        EUR-per-spec ratio for this candidate and the surviving-pool
        median, and explain whether the candidate sits inside the
        ±{ratio_pct}% sanity band. If not applicable, state why
        (e.g. service line item with no numeric lock-on).
      RECOMMENDED USE: tell the buyer how to use this row — as a primary
        anchor, secondary corroboration, or last-resort sanity check —
        and quantify the confidence. End with a single declarative
        sentence the buyer can stand behind in a meeting.

    The reasoning is the ONE thing a non-technical reviewer will read.
    Make every sentence count, cite numbers, do not hedge with words like
    "appears to be" or "likely". State the fit directly.

For EACH rejected row, produce a strong, defensible justification (not a
one-liner). Buyers must see WHY this row was thrown out so they can
challenge or accept the decision:
  - rule: one of CRITICAL_BARRIER | PRODUCT_TYPE | RATIO | ABSENCE |
          PLAYBOOK | OTHER (the rule that triggered the rejection).
  - disqualifier_attr: the specific attribute / level / ratio that broke.
  - source_value, candidate_value, unit, delta_pct: name the gap
    numerically when applicable; null for categorical mismatches.
  - reasoning: 5–8 sentences, again structured into four named beats:

      PRIMARY DISQUALIFIER: name the rule + attribute that broke. Quote
        source and candidate values with units. State the delta % or the
        family-boundary nature of the mismatch.
      QUANTIFIED GAP: explain HOW FAR outside the acceptable band the
        candidate sits — e.g. "21.4% over the strict ±10% tonnage band,
        more than 2× the tolerance".
      WHY THIS BLOCKS BENCHMARKING: explain in business terms what
        misleading effect including this candidate would have on the
        derived benchmark price — e.g. "anchoring a 1400T quote on a
        1700T historical would inflate the benchmark by roughly 20%
        based on the EUR-per-tonne curve for this drive family".
      WHAT WOULD QUALIFY: name the attribute change that would have
        turned this into a valid comparable — e.g. "a same-period
        purchase between 1260T and 1540T from a hydraulic press would
        have been a valid moderate match". This audit closer makes the
        rejection inspectable.
...
{{
  ...
  "selected": [
    {{
      ...
      "matched_attrs": [
        {{
          "name": "<canonical attr name e.g. tonnage>",
          "src": <value or string>,
  ...
  "rejected": [
    {{
      "purchase_dtl_id":    <int>,
      "rule":               "CRITICAL_BARRIER|PRODUCT_TYPE|RATIO|ABSENCE|PLAYBOOK|OTHER",
      "disqualifier_attr":  "<attr / level / ratio name>",
      "source_value":       <value or null>,
      "candidate_value":    <value or null>,
      "unit":               "<unit or null>",
      "delta_pct":          <number to 1 decimal or null>,
      "reasoning":          "PRIMARY DISQUALIFIER: ... QUANTIFIED GAP: ... WHY THIS BLOCKS BENCHMARKING: ... WHAT WOULD QUALIFY: ..."
    }}
  ]
}}
"""
...
Process — follow EVERY step:
  ...
  STEP 3 — PER-ROW REASONING (selected).
    For every survivor, emit the full `agent_reasoning` payload as
    specified in Part C — matched_attrs with src/cand/unit/delta_pct (one
    decimal, round-half-to-even), spec_to_price_ratio, penalties,
    confidence, verdict, and a 5–8 sentence `reasoning` that quotes
    numbers (e.g. "tonnage 2050T vs source 2000T, 2.5% delta, inside
    ±{critical_pct}% strict band"). Use the named PRODUCT FIT / SPEC
    ALIGNMENT / PRICE COMPARABILITY / RECOMMENDED USE structure.

  STEP 4 — PER-ROW REASONING (rejected).
    For every rejected candidate, populate disqualifier_attr +
    source_value + candidate_value + unit + delta_pct AND a 5–8 sentence
    `reasoning` using the PRIMARY DISQUALIFIER / QUANTIFIED GAP / WHY THIS
    BLOCKS BENCHMARKING / WHAT WOULD QUALIFY structure. No one-liners.

  STEP 5 — COMPLETENESS VERIFICATION (run this LAST):
    Count the total candidates received = {n_candidates}.
    Verify: |selected| + |rejected| MUST equal {n_candidates}. If your
    output drops below that count, you have silently lost candidates —
    go back to Step 0, identify the missing IDs, and emit them. The
    pipeline depends on every candidate having an explicit verdict.

  STEP 6 — ORDERING.
    Sort `selected` by rank ascending (1 = best). Among ties on every
    rule, sort by ascending purchase_dtl_id. Sort `rejected` by ascending
    purchase_dtl_id. This guarantees deterministic output across runs.

If the pool yields fewer than {top_k} valid matches that survive the
strict band, return fewer rather than padding with marginal ones. Returning
NULL or a short list is a correct answer when the historical pool is thin —
do NOT invent a comparable just to fill the `selected` array.
...
def _llm_rank_candidates(
    ...
    try:
        ...
        try:
            ...
            # If truncation broke the JSON we still surface usage stats so the
            # caller can persist them. Treat as empty verdicts.
            logger.warning(
    ...
    try:
        for raw_row in rows:
            ...
            try:
                ...
                # LLM completeness check — if the LLM silently dropped any
                # candidate (broke prompt STEP 5), surface those IDs as
                # LLM_DROPPED rejects so the funnel stays mathematically
                # consistent: sent_to_llm == selected ∪ rejected.
                emitted_sel_ids = {
                ...
                if dropped_silently and agent_reasoning_payload:
                    logger.warning(
                        f"[V2 {pr_no}] dtl_id={dtl_id} LLM silently dropped "
                        f"{len(dropped_silently)} candidate(s) sent to it (broke prompt "
                        f"STEP 5 completeness) — surfacing as LLM_DROPPED rejects: "
                        f"{dropped_silently}"
                    )
                    agent_reasoning_payload.setdefault("rejected", [])
                    for missing_id in dropped_silently:
                        agent_reasoning_payload["rejected"].append({
                            "purchase_dtl_id":   missing_id,
                            "rule":              "LLM_DROPPED",
                            "disqualifier_attr": None,
                            "source_value":      None,
                            "candidate_value":   None,
                            "unit":              None,
                            "delta_pct":         None,
                            "reasoning": (
                                "PRIMARY DISQUALIFIER: This candidate was sent to the LLM "
                                "ranker but did not appear in its selected or rejected "
                                "arrays. The pipeline completeness check inserted it here "
                                "automatically. QUANTIFIED GAP: not evaluated by the LLM. "
                                "WHY THIS BLOCKS BENCHMARKING: without an LLM verdict this "
                                "DTL cannot be used as a comparable. WHAT WOULD QUALIFY: "
                                "re-run with a larger bench_llm_shortlist_size or "
                                "investigate the LLM's STEP 5 completeness compliance."
                            ),
                        })
                funnel["llm_selected"]         = sorted(emitted_sel_ids)
                funnel["llm_rejected"]         = sorted(emitted_rej_ids | set(dropped_silently))
                funnel["llm_dropped_silently"] = dropped_silently
                # Copy token_usage from the agent reasoning into the funnel
                # so the DB row carries both the funnel and the budget data.
                token_usage_for_funnel = (
                    agent_reasoning_payload.get("token_usage")
                    if isinstance(agent_reasoning_payload, dict) else None
                )
                funnel["token_usage"] = token_usage_for_funnel
                truncated_flag = bool(token_usage_for_funnel and token_usage_for_funnel.get("truncated"))
                funnel["truncated"]  = truncated_flag
                ...
                logger.info(
                    ...
                    + (
                        f"  [!!! TRUNCATED: output {token_usage_for_funnel.get('output_tokens')}"
                        f"/{token_usage_for_funnel.get('max_output_tokens')} tokens, "
                        f"finish_reason={token_usage_for_funnel.get('finish_reason')!r} !!!]"
                        if truncated_flag and token_usage_for_funnel else ""
                    )
                )
