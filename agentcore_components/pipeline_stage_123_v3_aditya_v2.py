...
4. IMPORTANT ATTRIBUTES ARE PENALTIES.
   ...
   important mismatches start to be disqualifying. An "important" attribute
   present on the candidate but entirely absent from the source counts as a
   100% mismatch penalty. Two such absences are disqualifying.

5. BRAND SIMILARITY IS NOT RELEVANCE. Same brand alone is not a boost. Specs first.

6. RECENCY TIE-BREAK. Among candidates that survive rules 1-3, prefer more
   recently created PRs (closer to source date = lower inflation correction).
...
Return exactly K survivors (or fewer if the pool truly has fewer matches —
do NOT pad with weak matches). Every candidate the model evaluated must
appear in either `selected` or `rejected` — no candidate may silently
disappear.

For EACH selected row, produce a rich `agent_reasoning` payload so a buyer
can audit the match without re-running the model:
  - matched_attrs: the critical / important attributes you compared, with
    BOTH the source value and the candidate value, the unit, the delta %,
    the importance you assessed, and a per-attribute PASS / PENALTY verdict.
  - spec_to_price_ratio: only when meaningful (a critical numeric attr +
    a unit_price_eur both exist). Include the ratio, the surviving-pool
    median you used, and PASS / DOWNRANK / REJECT verdict.
  - penalties: short bullet-style strings for every soft penalty (important
    mismatches, brand differences flagged informationally, age, etc.).
  - confidence: 0.0–1.0 — your confidence that this candidate is a valid
    price benchmark, not just topically similar.
  - verdict: STRONG_MATCH | MODERATE_MATCH | WEAK_MATCH.
  - reasoning: 2–4 plain sentences a procurement manager could paste into
    a justification email. Reference numbers, not adjectives.

For each rejected row include `rule` (CRITICAL_BARRIER | PRODUCT_TYPE |
RATIO | ABSENCE | PLAYBOOK | OTHER) and a one-line reason naming the
specific attribute / level / ratio that triggered rejection.
...
{{
  ...
  "selected": [
    {{
      ...
      "penalties": ["<short bullet>", "..."],
      "confidence": <0.0-1.0>,
      "verdict": "STRONG_MATCH|MODERATE_MATCH|WEAK_MATCH",
      "reasoning": "<2-4 sentence buyer-facing justification>"
    }}
  ],
  "rejected": [
    {{
      ...
      "reason": "<short — name the attribute / level / ratio>"
    }}
  ]
}}
"""
...
Process:
  1. Read SOURCE.purchase_category_llm (and item_level chain) and pick the
     matching Part B playbook. State it in the top-level `category_playbook`.
  2. Walk EVERY candidate through Part A (universal rules) then the playbook.
  3. For every survivor, emit the full `agent_reasoning` payload as specified
     in Part C — matched_attrs with src/cand/unit/delta_pct, spec_to_price_ratio,
     penalties, confidence, verdict, and a 2-4 sentence `reasoning` that quotes
     numbers (e.g. "2050T vs source 2000T, 2.5% delta, within ±10% band").
  4. Every candidate must end up in `selected` OR `rejected` — none may
     vanish. If the pool yields fewer than {top_k} valid matches, return
     fewer rather than padding with weak matches.
