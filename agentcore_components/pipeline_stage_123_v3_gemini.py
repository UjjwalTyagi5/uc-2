...
2. **Quotation** — A SINGLE vendor's price offer / response to an RFQ (vendor → buyer such as Motherson)
   ...
   CRITICAL DISAMBIGUATION:
     ...
     . There can be many vendors/supplier not just HAITIAN, Engel, Sumitomo, KraussMaffei, Arburg etc. these are just examples.
      • **ARC / Price List Reports:** Documents titled "ARC Price List", "Annual Rate Contract", or "Price List Item Report" that list specific parts and their corresponding fixed prices from ONE supplier are Quotations. They may look like system reports but serve as the commercial offer.
...
def _extract_for_classification(file_bytes: bytes, filename: str) -> tuple:
    """Returns (text_content: str, images_b64: list[str], metadata_str: str)."""
    import os, io
    ext      = os.path.splitext(filename.lower())[1]
    ...
    return "[Unsupported file type]", [], meta_str
        ...
        for idx, sheet in enumerate(sheets):
            try:
                ...
                if idx < _MAX_SHEETS and non_empty > 0:
                    ...
                    try:
                        # Try to detect if first row is a header (mostly strings)
                        first_row = df.iloc[0]
                        ...
                        parts.append(data_df.fillna("").astype(str).to_markdown(index=False, headers=hdrs))
                    except Exception:
                ...
                pass
        return "\n".join(parts)[:_MAX_CLASSIFY_CHARS], [], meta_str
    except Exception as exc:
        return f"[Excel extraction error: {exc}]", [], meta_str
...
def _extract_csv_classify(file_bytes, meta_str):
    ...
    try:
        ...
        return text[:_MAX_CLASSIFY_CHARS], [], meta_str
    except Exception as exc:
        return f"[CSV extraction error: {exc}]", [], meta_str
...
def _extract_pdf_classify(file_bytes, filename, meta_str):
    ...
    try:
        ...
        with pdfplumber.open(buf) as pdf:
            ...
            return combined[:_MAX_CLASSIFY_CHARS], [], meta_str
    except Exception:
...
def _extract_pdf_fitz_classify(file_bytes, filename, meta_str):
    ...
    try:
        ...
        return f"[PDF extraction error: {exc}]", [], meta_str
...
def _extract_word_classify(file_bytes, filename, meta_str):
    ...
    try:
        ...
        # Fallback to fitz for better text extraction if docx fails
        if len(text.strip()) < 100:
            try:
                import fitz
                fdoc = fitz.open(stream=file_bytes, filetype="docx")
                ftext = ""
                for page in fdoc:
                    ftext += page.get_text()
                fdoc.close()
                if len(ftext.strip()) > len(text.strip()):
                    text = ftext
                    meta_str += "- text source: fitz (docx fallback)\n"
            except Exception:
                pass

        # Always check for images in word/media/ for visual context
        images_b64 = []
        try:
            with zipfile.ZipFile(io.BytesIO(file_bytes)) as z:
                imgs = sorted([n for n in z.namelist() if n.startswith("word/media/")])
                if imgs:
                    # Take top 3 largest images
                    imgs_sorted = sorted(imgs, key=lambda n: z.getinfo(n).file_size, reverse=True)[:3]
                    for img_name in imgs_sorted:
                        ...
                        images_b64.append(base64.b64encode(buf2.getvalue()).decode())
        except Exception:
            pass

        if not text.strip() and not images_b64:
            # Last resort: render the docx as images using fitz
            return _extract_fitz_classify(file_bytes, filename, meta_str)

        return text[:_MAX_CLASSIFY_CHARS], images_b64, meta_str
    except Exception as exc:
        return f"[Word extraction error: {exc}]", [], meta_str
    ...
    *,
    ...
    def _render_with_pdfplumber() -> list[str]:
        ...
        with pdfplumber.open(_io.BytesIO(file_bytes)) as pdf:
            total = len(pdf.pages)
            # Smart selection: First 2 pages + Last 1 page
            if total <= 3:
                idxs = list(range(total))
            else:
                idxs = sorted({0, 1, total - 1})
...
def _classify_file(llm, file_bytes: bytes, filename: str, prompts: dict | None = None) -> tuple:
    ...
    text_content, images_b64, meta_str = _extract_for_classification(file_bytes, filename)
    ...
    try:
        if images_b64:
            # Hybrid Prompt: Use the image template but inject the extracted text if available
        ...
        # --- ESCALATION LOGIC ---
        # If classification is 'Other' or confidence is low, and we have a way to 
        # get more visual context, escalate to vision.
        needs_escalation = (raw_cls == "Other" or confidence < 0.80)
        
        if needs_escalation and not images_b64:
            vision_samples_b64 = []
            
            if file_type == "pdf":
                # Multi-page PDF escalation (First 2 + Last 1 handled in _render)
            ...
            elif file_type in ("word", "pptx"):
                # Escalation for Word/PPTX: Try to extract embedded images 
                # even if text extraction previously succeeded but gave low confidence.
                import base64 as _b64
                from PIL import Image
                try:
                    with zipfile.ZipFile(_io.BytesIO(file_bytes)) as z:
                        prefix = "word/media/" if file_type == "word" else "ppt/media/"
                        imgs = sorted([n for n in z.namelist() if n.startswith(prefix)])
                        if imgs:
                            # Take top 3 largest
                            imgs_sorted = sorted(imgs, key=lambda n: z.getinfo(n).file_size, reverse=True)[:3]
                            for img_name in imgs_sorted:
                                ...
                                if max(img.size) > 2048: img.thumbnail((2048, 2048), Image.LANCZOS)
                                buf2 = _io.BytesIO()
                                img.save(buf2, format="PNG")
                                vision_samples_b64.append(_b64.b64encode(buf2.getvalue()).decode())
                except Exception:
                    pass
            ...
            if vision_samples_b64:
                logger.info(
                    ...
                    f"{len(vision_samples_b64)} image(s) (conf={confidence:.2f}, cls={raw_cls})"
                )
                user_prompt = user_image_tmpl.format(
                    filename=filename, file_type=file_type, extra_metadata=meta_str
                )
...
def _llm_rank_candidates(
    ...
    (selected, rejected). On failure returns ([], []) so the caller falls
    back to Pinecone order."""
    if not candidates:
        return [], []
    from langchain_core.messages import HumanMessage, SystemMessage
    p = prompts or {}
