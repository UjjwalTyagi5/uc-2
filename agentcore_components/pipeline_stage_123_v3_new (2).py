...
Hard rules for the output:
  ...
  1. The `verdicts` array MUST contain exactly ONE row per candidate. If
     `n_candidates = N`, then `len(verdicts) == N`. No exceptions. Do NOT
     skip rows for "obviously wrong" candidates — emit a compact FAIL row
     for them. The compact form is cheap (~25 tokens per row); writing 100
     rows is trivial. Skipping rows breaks the pipeline.

  2. Each row in `verdicts` is COMPACT — no paragraphs. Fields:
       purchase_dtl_id, verdict (PASS|FAIL), rule, attr, src, cand, unit,
       delta_pct, note.
     `note` is a 10–15 word phrase ONLY when something beyond the obvious
     applies (e.g. "3K combi machine, multi-injection-unit family").
     Otherwise leave `note` as "" (empty string). Most FAIL rows should
     have `note = ""`.

  3. The `selected_detail` array contains the FULL reasoning ONLY for
     candidates whose `verdict == "PASS"` in `verdicts`. One detail row per
     PASS verdict. If no candidate passes, `selected_detail` is `[]`.

  4. Rejected items live ONLY in `verdicts` with verdict=FAIL. Do NOT
     create a separate "rejected_detail" array — the post-processor will
     synthesize a buyer-readable rejection sentence from the compact fields.

  5. Order: sort `verdicts` by ascending purchase_dtl_id. Sort
     `selected_detail` by rank (1 = best).

For the FAIL `note` field, focus on judgement-required cases ONLY. Examples:
  - "" (empty)  — when the rule + attr + delta_pct already tell the full
                   story (e.g. "tonnage 100T vs 1800T, delta_pct=94.4").
                   Most FAILs are like this.
  - "3K combi machine, multi-injection-unit family"
  - "spare-part priced at 13k EUR vs press at ~400k EUR — accessory drift"
  - "category-alias match but item_level_3 is a service, not equipment"
...
{{
  ...
  "verdicts": [
    {{
      "purchase_dtl_id": <int>,
      "verdict":         "PASS|FAIL",
      "rule":            "CRITICAL_BARRIER|PRODUCT_TYPE|RATIO|ABSENCE|PLAYBOOK|OTHER|null",
...
Process — follow EVERY step:
  ...
  STEP 3 — VERDICTS (mandatory for EVERY candidate).
    Walk every candidate in ascending purchase_dtl_id order. Emit ONE row
    in `verdicts` per candidate with the compact fields:
      purchase_dtl_id, verdict (PASS or FAIL), rule, attr, src, cand,
      unit, delta_pct (1 decimal, round-half-to-even), note.

    For verdict=FAIL where the rule + attr + delta_pct already tell the
    story (e.g. tonnage out of band, wrong item_level_3), leave `note=""`.
    Use `note` ONLY for judgement-required cases — multi-component family,
    accessory/spare detection, service vs equipment confusion, suspicious
    price/spec ratio. 10–15 words maximum.

    DO NOT write paragraphs in this step. DO NOT skip rows for "obviously
    wrong" candidates — emit a compact FAIL row for them. The compact form
    is cheap (~25 tokens per row); writing {n_candidates} rows is trivial.
    Skipping rows breaks the pipeline.

    `len(verdicts) MUST equal {n_candidates}`. If lower, you skipped rows.

  STEP 4 — DETAIL (only for verdict=PASS rows).
    ...
    If NO candidate passes (no within-band match exists), `selected_detail`
    is `[]`. Do NOT invent a comparable just to fill the array. Returning
    an empty `selected_detail` with a complete `verdicts` array is the
    correct answer when the historical pool is thin.

  STEP 5 — COMPLETENESS VERIFICATION (run this LAST).
    Count the total candidates received = {n_candidates}.
    Verify: `len(verdicts) == {n_candidates}` EXACTLY. If less, you silently
    lost candidates — go back to STEP 3, identify the missing
    purchase_dtl_ids, and emit a compact FAIL row for each. The pipeline
    depends on every candidate having an explicit verdict.
...
def _build_short_reject_reasoning(verdict_row: dict) -> str:
    ...
    Examples:
      rule=CRITICAL_BARRIER, attr=tonnage, src=1800, cand=100, unit=T,
      delta_pct=94.4, note=""
        → "REJECT — tonnage 100T vs source 1800T (94.4% delta exceeds
           strict band)."

      rule=PRODUCT_TYPE, attr=item_level_3, src="Injection Moulding
      Machine", cand="Rubber Injection Moulding Machine", note=""
        → "REJECT — product type mismatch (item_level_3: 'Injection
           Moulding Machine' vs 'Rubber Injection Moulding Machine')."

      Any rule with non-empty note
        → base sentence + " BUT " + note
    """
    ...
    if rule == "CRITICAL_BARRIER" and delta is not None and src is not None and cand is not None:
        unit_str = unit or ""
        base = (
            f"REJECT — {attr} {cand}{unit_str} vs source {src}{unit_str} "
            f"({delta}% delta exceeds strict band)."
        )
    elif rule == "PRODUCT_TYPE":
        base = (
        ...
        base = (
            f"REJECT — playbook family boundary "
            f"({attr or 'family'}: source={src!r} vs candidate={cand!r})."
        )
    else:  # OTHER or unknown
        if attr is not None and src is not None and cand is not None:
            base = f"REJECT — {attr}: source={src!r}, candidate={cand!r}."
        else:
            base = "REJECT — outside acceptable bounds (rule: OTHER)."

    if note:
        return f"{base} BUT {note}."
    return base
            ...
            for v in verdicts_arr:
                ...
                if verdict_value == "PASS":
                    ...
                    rejected.append({
                        ...
                        "note":               v.get("note") or "",
                        "reasoning":          _build_short_reject_reasoning(v),
                    })
        else:
