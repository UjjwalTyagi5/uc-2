...
def _get_client() -> tuple:
    """Returns (client, model_name) based on config. Cached."""
    ...
    if settings.llm_provider == "azure":
        ...
        _client_cache = (client, settings.azure_openai_deployment_name)
    else:
        client = OpenAI(api_key=settings.openai_api_key)
        _client_cache = (client, settings.openai_model)
...
def call_llm_vision(
    ...
    logger.info("Calling Vision LLM", model=model)
